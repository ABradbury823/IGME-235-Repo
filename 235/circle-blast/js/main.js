...
let levelNum = 1;
let paused = true;
let canShoot = false;   //can player shoot yet?
...
function gameLoop(){
    ...
    //move 10% of distance per update at 60FPS
    let sensitivity = 6 * dt;

    //lerp between x and y values
    let newX = lerp(ship.x, mousePos.x, sensitivity);
    let newY = lerp(ship.y, mousePos.y, sensitivity);

    //keep ship on screen by clamping position
    let w2 = ship.width/2;
    ...
    //get rid of dead bullets, dead circles and explosions
...
function createCircles(numCircles){
    //standard bouncing circles - yellow
    ...
    //orthogonal wrapping circles - green
    for(let i=0;i<numCircles/4;i++){	
...
function fireBullet(e){
    ...
    //cosole.log(`${mouseX}, ${mouseY}`);
    if(paused || !canShoot) return;

    let bulletOffset = 0;
    if(bulletNum > 1) bulletOffset = 10;

    for(let i = 0; i < bulletNum; i++)
    {
        let b = new Bullet(0xFFFFFF, (ship.x - bulletOffset) + (bulletOffset * i), ship.y);
        bullets.push(b);
...
function loadSpriteSheet(){
    //16 frames, each is 64x64px, second row
